...
/*
 * this function will initialize the data in a dynarray to NULL.
 * 
 * Params:
 *    da - dynarray to be made null
 */
void set_null(struct dynarray *da)
{
...
/*
 * this function is meant to set the new params for a
 * dynarray when its being rehashed
 *
 * Params:
 *    da - new array whose vals are being init'd
 *    new_size - new size of the hashtable
 */
void set_arr_params(struct dynarray *da, int new_size)
{
  if(da->data){ free(da->data); }
  da->data = malloc( sizeof(void*) * new_size );
...
/*
 * this function will return the capacity of a dynarray
 *
 * Params:
 *    da - dynarray to find capacity of
 */
